...
class ImageLogger(Callback):
    ...
    def on_train_batch_end(
        ...
        if batch_idx == 2000:
            for cond in conditions:
                ...
                self.log_img(pl_module, tmp_batch, batch_idx, cond=cond, split="train")
    ...
    def on_validation_batch_end(
        ...
        if not self.disabled and pl_module.global_step > 0:
            for cond in conditions:
                tmp_batch = deepcopy(batch)
                self.log_img(pl_module, tmp_batch, batch_idx, cond=cond, split="val")
...
if __name__ == "__main__":
    ...
    if opt.gpus:
        model = model.to(torch.device(f"cuda:{opt.gpus.split(',')[0]}"))
